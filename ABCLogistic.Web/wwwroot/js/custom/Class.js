var uploadedCategory = null;

$(function () {
    $("#frmUpdateCategory").on("submit", function () {
        $(".validation-summary-errors ul li").remove();
        $("#frmUpdateCategory").valid();
...
$(document).on("change", "#classFile", function () {
    var fileUpload = $("#classFile").get(0);
    ...
    if (!SearchExt(ext.toLowerCase(), imageExtensions)) {
        ...
        $('.upload_photo_btn #classFile').val('');
        e.preventDefault();
    }
    else {
        ...
        if (files.length > 0) {
            ...
            reader.onload = function (e) {
                ...
                $("#btn_save_pro").bind("click", function (event) {
                    uploadedCategory = $("#classFile").val();
                    $resize.croppie("result").then(function (canvas) {
                        ...
                        $('#div_pro_view').hide();
                        $('<i class="fa fa-times-circle remove_profile" onclick="RemoveCategory()"></i>').insertAfter('#pro_img');
                        $("#div_category").removeClass("displaynone");
...
function DeleteImg(id) {
    if (confirm("Are you sure? Do you want to delete this image?")) {
        $.post(SiteUrl + 'Class/DeleteImage', { id: id }, function (data) {
            if (data.Success) {
...
function RemoveCategory_Cancel() {
    $('#pro_img').attr('src', "").parent().addClass("displaynone");
    $("#div_category i").remove();
    $('.upload_photo_btn #classFile').val('');
    $("#div_category").addClass("displaynone");
    $("#div_upload_pro").removeClass("displaynone");
    $("#div_upload_pro_img").addClass("displaynone");
    $("#div_crop_pro").addClass("displaynone");
